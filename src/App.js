...
export default function App() {
  //Checking the authentication state from useAuth context
  const { authInfo } = useAuth();
  const isAdmin = authInfo.profile?.role === "admin";//if role is admin render the different UI

  if (isAdmin) return <AdminNavigator />;

  //else render the user UI
  return (
    <>
